...
soritok.com 용 server 블록은 [`deploy/soritok.nginx.conf`](deploy/soritok.nginx.conf) 에 있습니다.

```bash
# DNS: soritok.com (및 www) A레코드 → 서버 IP 먼저 등록
sudo cp deploy/soritok.nginx.conf /etc/nginx/sites-available/soritok
sudo ln -s /etc/nginx/sites-available/soritok /etc/nginx/sites-enabled/
sudo nginx -t && sudo systemctl reload nginx
sudo certbot --nginx -d soritok.com -d www.soritok.com   # HTTPS 자동 설정
```
