 ...
 * 해수어·무척추 전용 규칙.
 *
 * 담수어와 달리 해수는 잘못된 정보가 곧바로 폐사로 이어진다.
 * 특히 특수 먹이(만다린·하퀸쉬림프), 리프 안전성, 독성 종은
 * 초보자가 모르고 들이면 생물이 죽거나 사람이 다친다.
 */
const SALT_RULES = `

[해수어·무척추 전용 규칙 — 반드시 지킬 것]

■ 수질 지표
- ph 필드는 해수 기준으로 쓴다. 자연해수는 pH 8.1~8.4 다. 담수 수치(6.5~7.5)를 쓰지 마라.
- detailCare 첫 문단에 **비중(염분)** 을 반드시 명시한다. 어류 전용 수조는 보통 1.020~1.025,
  산호·무척추를 함께 두는 리프 수조는 1.024~1.026 범위로 좁게 유지한다.
- 해수는 담수보다 수질 완충이 어렵고 사이클링(생물학적 여과 확립)에 더 오래 걸린다는 점을 다룬다.

■ 먹이 — 가장 위험한 항목. 대충 쓰지 마라
- 특수 먹이만 먹는 종은 그 사실을 diet 와 detailCare 에 **경고 수준으로 분명히** 적는다.
  예) 만다린류: 살아있는 코페포드를 주로 먹어, 성숙한 라이브락/리퓨지움 없이는 아사하기 쉽다
  예) 하퀸 쉬림프: 불가사리만 먹으므로 먹이용 불가사리를 계속 공급해야 한다
  예) 오렌지스팟 파일피시: 아크로포라 폴립을 주로 먹어 장기 사육이 매우 어렵다
  예) 무어리시 아이돌: 해면 등 편식이 심해 사육 난이도가 최상급이다
  예) 코퍼밴드 나비고기: 편식이 심해 먹이 붙이기가 어렵다
  예) 해마: 섭식이 느리고 활먹이·냉동 미시스에 의존해 빠른 어종과 합사하면 굶는다
- 이런 종은 difficultyLevel 을 4~5 로 매기고, 초보자에게 권하지 않는다고 명시한다.

■ 리프 안전성(reef safe) — companionship 에 반드시 포함
- 산호·무척추와 함께 둘 수 있는지를 분명히 밝힌다.
- 대형 엔젤·나비고기 다수는 산호 폴립을 뜯어 리프 수조에 부적합하다.
- 트리거·복어는 새우·소라 등 무척추를 잡아먹는다.
- 라이언피시·복어는 입에 들어가는 소형어를 포식한다.
- 탱류는 같은 속(특히 동종) 개체에 강한 공격성을 보인다.

■ 독성·안전 (사람에게 해가 되는 종은 detailCare 에 경고)
- 라이언피시·쏠배감펭: 등지느러미 가시에 독이 있어 쏘이면 심한 통증. 수조 관리 시 주의.
- 복어류: 내장 등에 테트로도톡신을 가진 종이 있다. 절대 식용 대상으로 다루지 마라.
- 성게: 가시에 찔릴 수 있다.

■ 무척추동물 전용
- 구리(Cu) 계열 약품은 무척추동물에 치명적이므로 같은 수조에서 쓸 수 없음을 명시한다.
- 입수 시 급격한 염분·수온 변화에 약해 드립 방식의 느린 적응(아클리메이션)이 필요하다.
- 말미잘은 강한 조명과 안정된 수질을 요구하고, 이동 중 파워헤드에 빨려들어가 다치는 사고가 흔하다.

■ 수조 크기
- 탱·대형 엔젤은 성체 크기가 커서 minTank 를 넉넉히 잡는다. 판매 시 작은 개체라도
  성체 기준으로 안내한다(예: 블루탱은 최소 450L 이상 권장).

■ 개체 출처
- 자연 채집 의존도가 높은 종은 그 사실을 적고, 양식(브리딩) 개체가 유통되는 종은
  양식 개체를 권장한다고 밝힌다. 클라운피시 대부분은 양식 개체가 흔하다.

■ 클라운피시 관련 흔한 오해
- 말미잘이 없어도 사육에 지장이 없다. 말미잘은 필수가 아니며, 오히려 말미잘 사육이
  더 어렵다는 점을 분명히 한다.
`
...
위 품종의 도감 카드를 형식대로 작성하라. ${
  ...
  // 해수·무척추는 전용 규칙을 덧붙인다 (먹이·리프안전성·독성이 담수와 완전히 다름)
  const isMarine = item.category === 'saltwater' || item.category === 'invertebrates'
  const system = isMarine ? SYSTEM + SALT_RULES : SYSTEM
